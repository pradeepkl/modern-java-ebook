...
public class FluentTransformations {
    ...
    void main() {
        ...
        // Step 1: removeIf — keep only available products
        catalogue.removeIf(p -> !p.available());

        // Step 2: replaceAll — apply 10% discount to every remaining product
        catalogue.replaceAll(p ->
            new Product(p.productId(), p.category(),
                        p.price() * 0.9, p.available()));

        // Step 3: sort — by category, then by price ascending
        catalogue.sort(
            Comparator.comparing(Product::category)
                      .thenComparingDouble(Product::price));

        // Step 4: forEach — log each transformed product
        catalogue.forEach(p ->
            log.info(p.category()
                + " | " + p.productId()
