...
public class StreamComposition {
    ...
    private static List<Order> getUKOrders() {
        return List.of(new Order("ORD-1002", "UK", 120.0), new Order("ORD-1005", "UK", 600.0));
    }

    private static List<Order> getUSOrders() {
        return List.of(new Order("ORD-1001", "US", 340.0), new Order("ORD-1004", "US", 80.0));
    }

    private static List<Order> getEUOrders() {
        return List.of(new Order("ORD-1003", "EU", 210.0));
    }

    private static String getRegionOverride(String orderId) {
        return orderId.equals("ORD-1001") ? "AMERICAS" : null; // null for most
    }
    ...
    void main() {
        ...
        List<String> regions = allRegions.stream()
                .map(o -> getRegionOverride(o.orderId()))
                .flatMap(Stream::ofNullable)
                .toList();
        LOG.info("Non-null region overrides: " + regions);

        // Stream.iterate — three-argument form (Java 9+) with built-in termination
        List<Integer> orderNumbers = Stream.iterate(1000, n -> n <= 1010, n -> n + 1).toList();
        LOG.info("Order numbers: " + orderNumbers);

        // Stream.generate — values from a Supplier, always paired with limit()
        ...
        // All regions count: 5
        // Valid statuses: [CONFIRMED, PENDING, SHIPPED, DELIVERED]
        // ofNullable(null) element count: 0
        // Non-null region overrides: [AMERICAS]
