 ...
 * Demonstrates: Factory pattern using constructor references in a registry
...
public class FactoryPattern {
    ...
    // Domain record shared across notifiers
    record Order(String orderId, String customerId,
                 double amount, String status, String region) {}

    // Notifier interface — fixed call site: notifier.send(order, message)
    interface Notifier {
        void send(Order order, String message);
    }

    // Each notifier is a record — no boilerplate class hierarchy needed
    ...
    public static void main(String[] args) {
        ...
        // Adding a new notifier: add one map entry, nothing else changes
        var registry = Map.<String, Supplier<Notifier>>of(
                "EMAIL", EmailNotifier::new,   // constructor ref as value
                "SMS",   SmsNotifier::new,
                "PUSH",  PushNotifier::new);

        // Fixed call site — the type is the moving part, not the invocation
        List.of("EMAIL", "SMS", "PUSH").forEach(type -> {
            var notifier = registry
                    .getOrDefault(type,
                            () -> { throw new IllegalArgumentException(
                                    "Unknown notifier type: " + type); })
                    .get(); // instantiate via stored constructor reference
            notifier.send(order, "Order confirmed.");
        });

        // Output:
        // INFO: EMAIL → C1: Order confirmed.
        // INFO: SMS → C1: Order confirmed.
        // INFO: PUSH → C1: Order confirmed.
    }
}