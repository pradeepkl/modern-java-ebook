// Java 25+
// Feature shown: stream single-use consumption and IllegalStateException, final in Java 21+
...
public class StreamConsumedOnce {
    ...
    void main() {
        ...
        LOG.info("Count from first terminal op: " + count);
        ...
        long count2 = orders.stream()
                .filter(o -> o.status().equals("CONFIRMED"))
                .count();
        LOG.info("Count (fresh stream): " + count2);

        List<Order> list2 = orders.stream()
                .filter(o -> o.status().equals("CONFIRMED"))
                .toList();
        LOG.info("List size (fresh stream): " + list2.size());

        // Output:
        // Count from first terminal op: 3
        // Stream already consumed — caught: IllegalStateException
        // Count (fresh stream): 3
        // List size (fresh stream): 3
    }
}